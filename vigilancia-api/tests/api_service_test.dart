...
class TestApiService {
  ...
  TestApiService({
    ...
    this.baseUrl = 'https://humberto.alwaysdata.net/api',
  });
...
void main() {
  ...
  group('getDevices()', () {
    ...
    test('lanza excepción con status 500', () async {
      ...
      expect(() => service.getDevices(token), throwsA(isA<Exception>()));
    });
  ...
  group('addDevice()', () {
    test('crea dispositivo y retorna datos con status 201', () async {
      ...
      final service = TestApiService(client: mockClient);
      final result = await service.addDevice(
        token,
        title: 'CAM-Nueva',
        subtitle: 'Zona Norte',
      );
  ...
  group('updateDevice()', () {
    test('actualiza dispositivo correctamente', () async {
      final mockClient = MockClient((request) async {
        ...
        return http.Response(
          jsonEncode({
            ...
            'device': {'id': 5, 'title': 'CAM-005', 'status': 'Mantenimiento'},
          }),
          200,
        );
      });
      ...
      final service = TestApiService(client: mockClient);
      final result = await service.updateDevice(
        token,
        5,
  ...
  group('deleteDevice()', () {
    test('elimina dispositivo correctamente', () async {
      ...
      await expectLater(service.deleteDevice(token, 3), completes);
    });
  ...
  group('Flujo completo de dispositivos', () {
    test('CRUD completo ejecuta correctamente en orden', () async {
      ...
      // 3. Actualizar
      final actualizado = await service.updateDevice(
        token,
        42,
        status: 'Inactivo',
      );
